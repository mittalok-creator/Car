...
const SheetsSync = (() => {
  ...
  const HEADER = ["Date", "Category", "Description", "Odometer (km)", "Litres", "Price/Litre", "Amount (INR)", "Payment Mode", "Garage", "Next Service Date", "Next Service Km", "Notes"];
  ...
  function rowToExpense(row, idx) {
    const [date, category, description, odometer, litres, pricePerLitre, amount, paymentMode, garage, nextServiceDate, nextServiceKm, notes] = row;
    return {
  ...
  async function pushExpenses(sheetId, expenses) {
    ...
    const range = `Sheet1!A1:L${rows.length}`;
  ...
  async function pullExpenses(sheetId) {
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Sheet1!A2:L10000`;
